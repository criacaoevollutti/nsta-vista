...
UPDATE public.profiles p SET position = r.rn FROM ranked r WHERE p.id = r.id AND p.position IS NULL;
...
BEGIN
  ...
  FOREACH _id IN ARRAY _profile_ids LOOP
    UPDATE public.profiles SET position = _i, updated_at = now() WHERE id = _id;
    _i := _i + 1;
  END LOOP;
  RETURN true;
END;
